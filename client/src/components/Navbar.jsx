  ...
  return (
    <>
      ...
      <nav className="bg-slate-900 border-b border-slate-800 sticky top-0 z-50 px-4 lg:px-8 py-3.5 flex justify-between items-center shadow-xl">
        ...
        <div className="hidden lg:flex items-center gap-6 text-xs font-extrabold shrink-0">
          <Link
            to="/"
            className={`transition px-2 py-1 rounded ${isActive('/') ? 'text-lime-accent bg-slate-800' : 'text-white hover:text-lime-accent'}`}
          >
            🏠 Overview
          </Link>
          <Link
            to="/citizen"
            className={`transition px-2 py-1 rounded ${isActive('/citizen') ? 'text-lime-accent bg-slate-800' : 'text-white hover:text-lime-accent'}`}
          >
            🏡 Resident Intake
          </Link>
          <Link
            to="/officer"
            className={`transition px-2 py-1 rounded ${isActive('/officer') ? 'text-lime-accent bg-slate-800' : 'text-white hover:text-lime-accent'}`}
          >
            👮 Officer Dashboard
          </Link>
          <Link
            to="/digital-twin"
            className={`transition px-2 py-1 rounded ${isActive('/digital-twin') ? 'text-lime-accent bg-slate-800' : 'text-white hover:text-lime-accent'}`}
          >
            🏙️ Digital Twin
          </Link>
          <Link
            to="/analytics"
            className={`transition px-2 py-1 rounded ${isActive('/analytics') ? 'text-lime-accent bg-slate-800' : 'text-white hover:text-lime-accent'}`}
          >
            📊 Analytics
          </Link>
          ...
          <Link
            to="/login"
            className="bg-lime-accent hover:opacity-90 text-slate-900 px-4 py-2 rounded-lg font-black transition shadow-lg"
          >
            🔑 Single Sign-On
          </Link>
        </div>
